...
void _initState(State s, InputStream input) {
  if (s.runningState != 0) {
  ...
  s.blockTrees[0] = 7;
  s.distRbIdx = 3;
  final maxDistanceAlphabetLimit = _calculateDistanceAlphabetLimit(
    0x7FFFFFFC,
    3,
    15 << 3,
  );
...
int _readSymbol(List<int> tableGroup, int tableIdx, State s) {
...
int _readBlockLength(List<int> tableGroup, int tableIdx, State s) {
  _fillBitWindow(s);
...
void _moveToFront(List<int> v, int index) {
  final value = v[index];
...
void _inverseMoveToFrontTransform(List<int> v, int length) {
...
void _checkDupes(List<int> symbols, int length) {
...
int _decodeContextMap(int contextMapSize, List<int> contextMap, State s) {
  _readMoreInput(s);
...
int _decodeBlockTypeAndLength(State s, int treeType, int numBlockTypes) {
...
void _decodeLiteralBlockSwitch(State s) {
  s.literalBlockLength = _decodeBlockTypeAndLength(
    s,
    0,
...
void _decodeCommandBlockSwitch(State s) {
  s.commandBlockLength = _decodeBlockTypeAndLength(
    s,
    1,
...
void _decodeDistanceBlockSwitch(State s) {
  s.distanceBlockLength = _decodeBlockTypeAndLength(
    s,
    2,
...
void _readNextMetablockHeader(State s) {
  ...
  if (s.expectedTotalSize > s.outputLimit) {
    throw const BrotliOutputLimitException();
  }
...
int _readMetablockPartition(State s, int treeType, int numBlockTypes) {
...
void _calculateDistanceLut(State s, int alphabetSizeLimit) {
...
void _readMetablockHuffmanCodesAndContextMaps(State s) {
  ...
  s.distanceBlockLength = _readMetablockPartition(
    s,
    2,
    s.numDistanceBlockTypes,
  );
  ...
  final numLiteralTrees = _decodeContextMap(
...
void _replicateValue(List<int> table, int offset, int step, int end, int item) {
  do {
    end -= step;
...
int _nextTableBitSize(List<int> count, int len, int rootBits) {
...
void _checkHealth(State s, int endOfStream) {
  if (s.endOfStreamReached == 0) {
    return;
  }
...
void _doFillBitWindow(State s) {
  s.accumulator =
      (BigInt.from(s.intBuffer[s.halfOffset++] & 0xffffffff) << _halfBitness) |
...
int _peekBits(State s) {
  return ((s.accumulator >> s.bitOffset) & BigInt.from(0xffffffff)).toInt();
}
...
void _copyBytes(State s, List<int> data, int offset, int length) {
  if ((s.bitOffset & 7) != 0) {
...
void _bytesToNibbles(State s, int byteLen) {
...
int _readInput(InputStream? src, List<int> dst, int offset, int length) {
  if (src == null) {
    return -1;
  }
...
class BrotliCodec extends Codec<List<int>, List<int>> {
  ...
  String decodeToString(List<int> encoded, {Encoding? encoding}) {
...
List<int> _decode(List<int> data, [List<int>? compoundDictionary]) {
...
/// Decodes Brotli while stopping as soon as [maxOutputBytes] is exceeded.
///
/// This entry point was added by c2pa_codec. The upstream decoder emits into
/// bounded chunks, so the limit is enforced during decompression rather than
/// after accumulating the complete output.
Uint8List decodeWithLimit(List<int> data, int maxOutputBytes) {
  if (maxOutputBytes < 0) {
  ...
  var outputLength = 0;
  var isLast = false;
  state.outputLimit = maxOutputBytes;
  _initState(state, InputStream(data));

  try {
    while (!isLast) {
      final remaining = maxOutputBytes - outputLength;
      final chunkLength = min(16384, remaining + 1);
      final chunk = Uint8List(chunkLength);
      state.outputOffset = 0;
      state.output = chunk;
      state.outputLength = chunk.length;
      state.outputUsed = 0;

      _decompress(state);

      final length = state.outputUsed;
      if (length > remaining) {
        throw const BrotliOutputLimitException();
      }
      if (length != 0) {
